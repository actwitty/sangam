...
//    aw_lib_console_log("DEBUG", "response:aw_api_facebook_get_profile : " + JSON.stringify(data));
...
function aw_api_facebook_get_feeds(feed_type, fn_cb){
  ...
  var url = aw_local_facebook_request_url_base[feed_type].
                                                          ...
                                                          replace("{FB_USER_ID}",
                                                              aw_api_facebook_get_visited_user_id());
  var timer = window.setTimeout(function(){
                                            aw_lib_console_log("WARN", "response failed:aw_api_facebook_get_feeds : " + feed_type);
                                            fn_cb("facebook", [], 2);
                                          }, 15000);
  $.getJSON(url,function(fb_data) {
    window.clearTimeout(timer);
  ...
  $.jsonp({
            ...
            complete:
                function(xOptions, textStatus) {
                  ...
                  if( id_list_data.data && id_list_data.data.length){
                    $.each( id_list_data.data, function ( key, post_data){
