 ...
 *
 */
function aw_get_channel_id_requested(){
  return $("#aw_js_ppm_channel_page_channel_id").val();
}
 ...
 *
 */
function aw_api_srv_resp_ppm_stm_chn_page_stream_list(params){
  var data = aw_awpi_serv_resp_data_for_get_request_in_params(params);
  
  var default_theme = aw_lib_get_default_channel_theme_for_category(); 
  var channel_theme = default_theme.thumb;

  $("#aw_js_ppm_channel_page_name").html(data.name);
  $(".aw_js_ppm_channel_page_theme_thumb").css("background-image", "url(" + channel_theme + ")");
  $("#aw_js_ppm_stm_channel_page_main_label").html("All " + data.name + " streams");
  ...
  $.each(data.stream, function(i, stream_info){
    if(stream_info.post){
      ...
      $('#aw_js_ppm_stm_channel_page_list').append(html);
      ...
      aw_api_ppm_cmn_more_cookie_set('AW_SRV_PPM_STM_CHN_PG_STREAM_LIST', stream_info.post.time);
  ...
  $('#aw_js_ppm_stm_stream_channel_page_div').find(".aw_js_ppm_loading_animation").hide();
}
 ...
 *
 */
function aw_ppm_stm_single_channel_get_stream(on_init){
  ...
  if( on_init ){
    ...
    aw_api_ppm_cmn_more_cookie_set('AW_SRV_PPM_STM_CHN_PG_STREAM_LIST', '');
    /* erase the timeline */
    $("#aw_js_ppm_stm_channel_page_list").html('');
  }
  $('#aw_js_ppm_stm_stream_channel_page_div').find(".aw_js_ppm_loading_animation").show();
  var params = {
                  'aw_srv_protocol_params' : {  
                                                id: aw_get_channel_id_requested(),
                                                updated_at : aw_api_ppm_cmn_more_cookie_get('AW_SRV_PPM_STM_CHN_PG_STREAM_LIST'),
...
$(document).ready(function(){
  $("#aw_js_ppm_stm_channel_page_data_more").click(function(){
    aw_ppm_stm_single_channel_get_stream(0);
  });
});
