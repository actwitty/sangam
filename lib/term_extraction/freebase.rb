    ...
    SKIP_TYPES = /\/type|\/common|\/base|\/people\/person|\/influence|\/user/
     ...
     def self.search_freebase(entities)
       ...
       entity_sets.each do |e_a|
         ...
         data.each do |k,v|
           result = v['result']
           name = result[0]['name'] if !result.blank?
           query = result[0]['search']['query'] if !result.blank?

           if !name.blank? #and (name.downcase == query.downcase)

             hash[name] = {} if hash[name].blank?
             hash[name][:name]=result[0]['name']
             hash[name][:id]=result[0]['search']['id']
      ...
      Rails.logger.error("[MODULE] [TEXT_EXTRACTION] [Freebase] [search_freebase] Rescue => ERROR => #{e.message}")
      return {}
    end
  end
end
