  ...
  module Services
    class Embedly
      ...
      def self.data_adaptor(response)
         ...
         h = {
           :provider => response["provider_url"], :description => response["description"], :title => response["title"],
      ...
      def self.resolve_links(links)
        ...
        Rails.logger.error("[LIB] [LINK_RESOLUTION] [EMBEDLY] [RESOLVE_LINKS] **** RESCUE **** #{e.message}")
        return {}
      end
    end
  end
