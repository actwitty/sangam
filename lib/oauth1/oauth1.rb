...
module Oauth1
  class OauthUtil
    ...
    def sign( params )
      ...
      Rails.logger.info("[LIB] [OAUTH1] [OauthUtil] [sign] entering #{params.inspect}")
      ...
      if !params[:service_opts].blank?
        @params.merge! params[:service_opts]
      end

      parsed_url = params[:parsed_url]
      ...
      Rails.logger.info("[LIB] [OAUTH1] [OauthUtil] [sign] leaving")
      self
    rescue => e
      Rails.logger.error("[LIB] [OAUTH1] [OauthUtil] [sign] ****RESCUE**** #{e.message} for #{params.inspect}")
      nil
    end
  end
end
