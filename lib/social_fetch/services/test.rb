       ...
       #OUTPUT => array of messages from test in descending order of creation time
        def pull_data(params)
          ...
          h = {:word => "stories",:category_id => "stories", :enrich => true,
                    :source_name => "test", :status_at_source => AppConstants.status_public_at_source, :source_uid => "2367263",}
          ...
          Rails.logger.info("[LIB] [SOCIAL_FETCH] [FETCHER] [TEST] [pull_data] leaving  #{params.inspect}")
          array
        rescue => e
          Rails.logger.error("[LIB] [SOCIAL_FETCH] [FETCHER] [TEST] [pull_data] **** RESCUE **** #{e.message} for #{params.inspect}")
        end

        def data_adaptor(params)
          Rails.logger.info("[LIB] [SOCIAL_FETCH] [FETCHER] [TEST] [data_adaptor] entering  #{params.inspect}")

          hash = {}
          links = []

          links = ::Api::Helpers::Parser.get_documents({:text =>params[:blob][:text]}) if !params[:blob][:text].blank?

          hash[:post] = params[:blob]
          hash[:post][:links] = links if !links.blank?

          Rails.logger.info("[LIB] [SOCIAL_FETCH] [FETCHER] [TEST] [data_adaptor] leaving  #{params.inspect} for #{links}")
          hash
        rescue => e
          Rails.logger.error("[LIB] [SOCIAL_FETCH] [FETCHER] [TEST] [data_adaptor] **** RESCUE **** #{e.message} for #{params.inspect}")
          {}
        end
      end
    end
