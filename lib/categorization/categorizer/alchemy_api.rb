    ...
    class AlchemyApi
      ...
      class << self
        ...
        def categorize_text(requests)
          ...
          Rails.logger.error("[MODULE] [CATEGORIZATION] [CATEGORIZER] [AlchemyApi] [categorize_text] => Rescue => ERROR => #{e.message}")
          return nil
        end

        def categorize_link(links)
          Rails.logger.info("[MODULE] [CATEGORIZATION] [CATEGORIZER] [AlchemyApi] [categorize_link]
                              entering => Number of link => #{links.size}")

          if links.blank?
            Rails.logger.info("[MODULE] [CATEGORIZATION] [CATEGORIZER] [AlchemyApi] [categorize_link]  => Return blank as no links ")
            return {}
          end
          ...
          Rails.logger.info("[MODULE] [CATEGORIZATION] [CATEGORIZER] [AlchemyApi] [categorize_link]=> #{hash.size}")

          hash
        rescue => e
          Rails.logger.error("[MODULE] [CATEGORIZATION] [CATEGORIZER] [AlchemyApi] [categorize_link] Rescue => ERROR => #{e.message}")
          return nil
        end
        ...
        def process_response(response)
          ...
          Rails.logger.info("[MODULE] [CATEGORIZATION] [CATEGORIZER] [AlchemyApi] [process_json] Rescue => ERROR => #{e.message}")
          return []
        end
      end
    end
