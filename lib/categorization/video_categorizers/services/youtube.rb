        ...
        def self.categorize(links)
          ...
          url_sets.each do |url_set|
            ...
            response_array.each do |response|
              ...
              data_adaptor(response[:response], hash, id_url_hash)
            end
          end
          ...
          Rails.logger.error("[MODULE] [CATEGORIZATION] [CATEGORIZER] [YOUTUBE] [categorize] **** RESCUE **** ==> #{e.message}")
          return {}
        end
      end
    end
